...
`SemanticLogos.jl` es un proyecto de NLP y humanidades digitales escrito en Julia. Su objetivo es estudiar cómo cambia el uso contextual de conceptos abstractos —por ejemplo, compasión, deber, existencia o alma— entre corpus pertenecientes a distintos periodos, autores o paradigmas culturales.
...
**v0.1.0 — Research prototype reproducible.**
...
unzip SemanticLogos.jl-v0.1.0.zip
cd SemanticLogos.jl
...
Los intervalos de confianza se estiman por **block bootstrap**, preservando parcialmente la dependencia local del texto.
...
### 5. Alineamiento entre corpus

Dos embeddings entrenados por separado no comparten necesariamente el mismo sistema de coordenadas. Por ello `SemanticLogos.jl` **no compara directamente** sus vectores.
...
La especificación original se conserva sin sustituirla en `references/project_specification.md`.
