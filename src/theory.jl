...
function replicator_rhs(x::AbstractVector{<:Real}, payoff_matrix::AbstractMatrix{<:Real})
    ...
    state = Float64.(x)
    all(value -> value >= 0.0, state) || throw(ArgumentError("x must be non-negative."))
    sum(state) > 0.0 || throw(ArgumentError("x must contain positive mass."))
    state ./= sum(state)
...
function replicator_step(x::AbstractVector{<:Real}, payoff_matrix::AbstractMatrix{<:Real}, dt::Real)
    ...
    state = Float64.(x)
    state ./= sum(state)
    next_state = state .+ Float64(dt) .* replicator_rhs(state, payoff_matrix)
