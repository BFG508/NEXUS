...
"""Return token sequences grouped by corpus while preserving document boundaries."""
function grouped_token_sequences(documents::Vector{LoadedDocument})
    groups = Dict{String,Vector{Vector{String}}}()
    for (group, group_documents) in grouped_documents(documents)
        groups[group] = [copy(document.tokens) for document in group_documents]
    end
    return groups
end

"""Flatten grouped tokens for backward-compatible descriptive operations only."""
function grouped_tokens(documents::Vector{LoadedDocument})
    groups = Dict{String,Vector{String}}()
    for (group, sequences) in grouped_token_sequences(documents)
        groups[group] = reduce(vcat, sequences; init = String[])
    end
    return groups
end
