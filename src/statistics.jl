...
function _sequence_blocks(token_sequences::Vector{Vector{String}}, block_size::Int)
    blocks = Vector{Vector{String}}()
    for tokens in token_sequences
        append!(blocks, _token_blocks(tokens, block_size))
    end
    return blocks
end

"""Resample blocks while retaining each sampled block as an independent sequence."""
function _resample_blocks(rng::AbstractRNG, blocks::Vector{Vector{String}})
    isempty(blocks) && return Vector{Vector{String}}()
    return [copy(blocks[rand(rng, eachindex(blocks))]) for _ in eachindex(blocks)]
end
...
function bootstrap_frequency(
    ...
    blocks = _sequence_blocks(token_sequences, block_size)
...
function bootstrap_semantic_shift(
    ...
    reference_blocks = _sequence_blocks(reference_sequences, block_size)
    target_blocks = _sequence_blocks(target_sequences, block_size)
    rng = MersenneTwister(seed)
    estimates = Float64[]
