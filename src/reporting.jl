...
function write_markdown_report(
    ...
    mkpath(dirname(path))
    open(path, "w") do io
        println(io, "# SemanticLogos.jl — Analysis report")
        println(io)
        ...
        println(io)
        println(io, "Frequencies are normalized to $(Int(round(config.frequency_scale))) tokens. Confidence intervals use block bootstrap.")
        println(io)
        frequency_table = [
            [
                string(row.group),
