...
### Fixed
- Required Procrustes alignments to have enough anchors for the actual embedding dimension and full numerical anchor rank.
- Added deterministic frequency-stability filtering and row-normalized anchor fitting.
- Rejected unsupported cross-language semantic comparisons in the monolingual baseline.
- Removed the implicit `dirname(dirname(config_path))` project-root assumption in favor of an explicit portable config root.
- Stopped semantic bootstrap from swallowing unexpected exceptions.
- Withheld semantic confidence intervals when too few valid bootstrap replicates survive.
- Strengthened raw corpus-manifest validation and normalized configured lexicon terms with the same tokenizer.

### Changed
- Reworked semantic bootstrap to build/alignment each resampled embedding pair once and reuse it across all concepts.
- Switched within-document resampling to circular moving blocks with exact document-length preservation.
- Added explicit configured comparison pairs.
- Increased CSV floating-point output precision and expanded run provenance with dirty-tree/root information.
- Reduced the synthetic demonstration embedding dimension to a scale supported by its small shared vocabulary.

### NEXUS
- Added repository-level `semantic_culture_profile.v1` export and explicit-policy ETHOS-to-EDEN adapters without adding integration code to the standalone ETHOS release.
- Migrated active documentation to the NEXUS English/GIDEON-style presentation while preserving historical release ZIPs unchanged.
...
### Changed
- Consolidated package metadata, scripts, tests, reports, documentation, citation information, fixtures, and repository paths under the canonical ETHOS identity.
- Defined the canonical acronym as **Evolution of Textual Humanism, Ontology and Semantics**.
- Preserved the original project specification for provenance.
